...
const EMOJIS = ['ðŸ–ï¸','ðŸš—','ðŸ ','ðŸ“±','âœˆï¸','ðŸ’°','ðŸŽ“','ðŸ’ª','ðŸ‹ï¸','ðŸŽ¸','ðŸ•','ðŸŒ']
...
export default function GoalsClient({ initialGoals }: { initialGoals: Goal[] }) {
  ...
  async function save() {
    ...
    if (!name || isNaN(t) || t <= 0) { alert('Preencha nome e valor alvo.'); return }
    setSaving(true)
    try {
      ...
      setShowModal(false); setName(''); setCur(''); setTarget(''); setEmoji('ðŸ’°')
      toast('Meta criada')
          ...
          return (
            <div key={g.id} className="card" style={{ padding: '18px' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '10px' }}>
                ...
                <button onClick={() => del(g.id)} style={{ width: '22px', height: '22px', background: 'none', border: '1px solid var(--border)', borderRadius: '5px', cursor: 'pointer', color: 'var(--text3)', fontSize: '10px', display:'flex',alignItems:'center',justifyContent:'center' }} onMouseEnter={e=>(e.currentTarget.style.color='var(--red)')} onMouseLeave={e=>(e.currentTarget.style.color='var(--text3)')}>âœ•</button>
              </div>
              ...
              <div style={{ fontSize: '10px', color: 'var(--text3)' }}>{pct}% concluÃ­do</div>
            </div>
          )
        })}
          ...
          <div className="modal-box" style={{ width: '420px' }}>
            ...
            <div style={{ padding: '18px 22px' }}>
              <div style={{ marginBottom: '13px' }}>
                ...
                <input className="fi" value={name} onChange={e => setName(e.target.value)} placeholder="Ex: Reserva de emergÃªncia..." />
              </div>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '10px', fontWeight: 600, letterSpacing: '1px', textTransform: 'uppercase', color: 'var(--text3)', marginBottom: '8px' }}>Ãcone</label>
