...
export default function GoalsClient({ initialGoals }: { initialGoals: Goal[] }) {
  ...
  async function save() {
    ...
    try {
      ...
      const { data } = await supabase.from('goals').insert({ user_id: user!.id, name, emoji, current_value: parseFloat(cur) || 0, target_value: t }).select().single()
      setGoals(prev => [...prev, data])
      setShowModal(false); setName(''); setCur(''); setTarget(''); setEmoji('💰')
      toast('Meta criada')
